-- =============================================
-- OPTIMIZED VERSION OF OnlineBookedHalls4
-- Original procedure with significant performance improvements
-- =============================================
...
BEGIN
    ...
    -- Create temp table with all needed columns and primary key for performance
    CREATE TABLE #BookedHalls (
        ...
        ResStatus int,
        PRIMARY KEY CLUSTERED (HallNumberId, ArrivalDate, DepartureDate)
    );

    -- Pre-calculate time values
    DECLARE @StartTime varchar(5) = CONVERT(varchar(5), @AStartDateTime, 108);
    DECLARE @EndTime varchar(5) = CONVERT(varchar(5), @AEndDateTime, 108);

    -- =============================================
    -- 1. Get all halls (optimized with direct insert)
    -- =============================================
    ...
    -- Update MaxPax from tablesetup if needed (set-based instead of cursor)
    UPDATE h
    ...
    -- Apply subtype filter (set-based instead of cursor)
    IF @SubTypeTxt <> ''
    BEGIN
    ...
    -- =============================================
    -- 2. Get all bookings (optimized with single query)
    -- =============================================
    INSERT INTO #BookedHalls (
    ...
    -- =============================================
    -- 3. Add hall images (set-based instead of cursor)
    -- =============================================
    UPDATE h
    SET HallImage = (
